    ...
    <div className="p-4 md:p-8">
      ...
      {showForm && (
        <div className="card p-5 sm:p-6 mb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button className="btn-primary w-full sm:w-auto" onClick={handleCreate}>Shop aanmaken</button>
            <button className="btn-secondary w-full sm:w-auto" onClick={() => setShowForm(false)}>Annuleren</button>
          </div>
        </div>
      )}
          ...
          {shops.map(shop => (
            <div key={shop.id} className="card p-5">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-1.5">
                  {shop.etsy_shop_id ? (
                    ...
                    <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full font-medium">⚠️ Niet</span>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                {!shop.etsy_shop_id ? (
                  <button className="btn-primary w-full" onClick={() => handleEtsyOAuth(shop)}>
                    🔗 Koppel met Etsy OAuth
                  </button>
                ) : (
                  <button className="btn-secondary w-full" onClick={() => handleEtsyOAuth(shop)}>
      ...
      <div className="mt-8 card p-5 bg-blue-50 border-blue-100">
        ...
        <ol className="text-sm text-blue-700 space-y-1 list-decimal list-inside">
          ...
          <li>Stel de Redirect URI in op: <code className="bg-blue-100 px-1 rounded text-xs break-all">https://zvelo-agent.onrender.com/auth/etsy/callback</code></li>
          <li>Voeg ETSY_CLIENT_ID en ETSY_CLIENT_SECRET toe in Render Dashboard</li>
